...
| Medium | Frameless chrome vs platform window behavior | `MainWindow`, MDI subwindows, and docks now replace native title bars with Resources chrome widgets and manual hit testing. | Drag, resize, minimize/maximize, focus, accessibility, and window-manager behavior can diverge across macOS, Linux, and Windows. | Validate every chrome action per platform before publishing; keep lifecycle authority in `DeviceSession` and keep Resources chrome generic. |
